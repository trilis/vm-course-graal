...
public abstract class LamaShortCircuitNode extends LamaExpressionNode {
    ...
    public LamaShortCircuitNode(LamaExpressionNode left, LamaExpressionNode right) {
        this.left = left;
        this.right = right;
    }
    ...
    public final long executeLong(VirtualFrame frame) {
        boolean leftValue;
        try {
            leftValue = left.executeLong(frame) != 0;
        } catch (UnexpectedResultException e) {
            throw LamaException.typeError(this, e.getResult(), null);
        }
        boolean rightValue;
        try {
            if (evaluateRightProfile.profile(isEvaluateRight(leftValue))) {
                rightValue = right.executeLong(frame) != 0;
            } else {
                rightValue = false;
            }
        } catch (UnexpectedResultException e) {
            throw LamaException.typeError(this, leftValue, e.getResult());
        }
        return execute(leftValue, rightValue) ? 1 : 0;
    }
