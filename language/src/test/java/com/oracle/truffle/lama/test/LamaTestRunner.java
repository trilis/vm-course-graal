...
import com.oracle.truffle.lama.test.LamaTestRunner.TestCase;

public class LamaTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".lama";
    ...
    public LamaTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        LamaTestSuite suite = c.getAnnotation(LamaTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", LamaTestSuite.class.getSimpleName(), c.getName(), LamaTestRunner.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != LamaTestSuite.class) {
    ...
    private static void run(Context context, Path path, PrintWriter out) throws IOException {
        try {
            /* Parse the Lama source file. */
            Source source = Source.newBuilder(LamaLanguage.ID, path.toFile()).build();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        LamaTestRunner suite = new LamaTestRunner(testClass);
        if (args.length > 0) {
